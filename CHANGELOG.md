...
### 🐛 FIX: Squib Menu Visibility Toggle
- **✅ Исправлено меню редактора врезки**: Теперь появляется при клике на врезку
  - Исправлен атрибут в `format/config.ts`: `data-type="squib"` вместо `data-align`
  - Обработчик кликов теперь корректно находит элементы врезок
