...
- ⚡ **Оптимизированы публичные API** в `src/graphql/api/public.ts`
  ...
  - `loadAuthors()` - кешированные загрузки авторов (30м браузер, 1ч CDN)
  - `loadShouts()` - кешированные загрузки статей (15м браузер, 30м CDN)
  - Автоматическое определение статичности запросов для безопасного кеширования

- 🎨 **Точное восстановление типографики aside согласно Figma макету** 
  - **Правильный font-weight**: Изменен с `700` (bold) на `500` (medium) согласно макету
  - **Компактные размеры**: h4 и h5 установлены в `1.6rem` и `1.4rem` соответственно
  - **Letter-spacing**: Добавлен `-1%` для корректного межбуквенного интервала
  - **Line-height**: Установлен точный `1.25` вместо `1.2`/`1.3` для плотности макета
  - **Шрифт Muller**: Явно указан `font-family: Muller, Arial, Helvetica, sans-serif`
  - **Цвета**: Обновлены до `#141414` согласно Figma спецификации
  - **Компактные отступы**: Уменьшены padding и margin для соответствия макету
  - **Заголовки секций**: Добавлен стиль `.sectionTitle` для uppercase заголовков (`font-size: 1.2rem`, `font-weight: 700`, `letter-spacing: 8%`)
  - **AsideSection**: Обновлен `border-radius: 8px`, уменьшены отступы до `1.2rem`/`1.6rem`

- 🎨 **Восстановлена типографика aside согласно дизайн-системе**
  - **h4 заголовки**: Увеличены до `2.2rem` (lg: `2.4rem`) с `font-weight: 700`
  - **h5 заголовки**: Установлены в `1.8rem` (lg: `2rem`) с `font-weight: 600`
  - **Текстовые элементы**: `1.5rem` (lg: `1.6rem`) с правильными цветами
  - **Ссылки**: `1.4rem` (lg: `1.5rem`) с hover эффектами
  - **AsideSection**: Обновлены отступы `1.8rem` (lg: `2.2rem`), добавлены тени и borders
  - **AsideTitle**: Приведен к `2.8rem` (lg: `3.2rem`) согласно иерархии заголовков
...
- 🎨 **Улучшено использование ширины экрана для переключателя режимов** - оптимизированно использование пространства
  - **Увеличена ширина**: FeedSwitcher теперь занимает больше места (до 600px вместо 400px) для лучшего использования центральной колонки
  - **Больше кнопки**: Увеличены размеры кнопок - `padding: 1.2rem 2rem` и `font-size: 1.5rem` для лучшей читаемости и удобства
  - **Центрирование контента**: Добавлен `min-height: 4.8rem` с flexbox центрированием для единообразной высоты
  - **Гибкая раскладка**: FeedSwitcher теперь занимает `flex: 1` в контейнере фильтров, а фильтры `flex: 0 0 auto`
  - **Адаптивность**: На мобильных устройствах остается полная ширина с соответствующим уменьшением размеров
  - **Лучший баланс**: Эффективнее использует доступное пространство центральной колонки (7/12 ширины экрана)

- 📱 **Революционная оптимизация layout ленты для полного использования ширины экрана**
  - **Переход от Bootstrap Grid к Custom Flexbox Layout**: заменен ограничивающий `container-fluid` на полноширинный custom layout
  - **Адаптивные пропорции**: левый сайдбар (280px), основной контент (flex: 1), правый сайдбар (300px)
  - **Полное использование ширины экрана**: убраны все ограничения `max-width`, контент теперь растягивается на всю ширину
  - **Оптимизированный FeedSwitcher**: увеличена `max-width` до 600px для лучшего заполнения пространства
  - **Улучшенная мобильная адаптация**: вертикальный layout на мобильных устройствах, скрытие правого сайдбара
  - **Производительность**: flexbox layout быстрее Bootstrap grid системы

### Fixed
- 🚀 **Исправлена работа FeedSwitcher** - переключение между режимами ленты теперь работает корректно
  - **Логика режимов**: Исправлена `feedByMode` функция для правильного отображения текущего режима
  - **Инициализация данных**: Добавлена автоматическая инициализация фида данными из props при первой загрузке
  - **Автозагрузка**: Улучшен эффект автоматической загрузки данных при смене режима
  - **Кеширование**: Добавлена проверка наличия данных перед загрузкой - избегается повторная загрузка
  - **Персональные ленты**: Правильная обработка авторизованных лент (followed, discussed, coauthored)
  - **Публичные режимы**: Корректное переключение между recent, hot, top режимами
  - **Логирование**: Добавлено подробное логирование для отладки процесса переключения режимов
  - **Синхронизация**: Улучшена синхронизация между URL, состоянием контекста и отображением данных
  
- 🔧 **Обновлено использование нового API** для кешируемых загрузчиков
  - **SearchModal**: Исправлены вызовы `loadAuthorsSearch` с правильной структурой аргументов `{ text, limit, offset }`
  - **AuthorsProvider**: Обновлен поиск авторов для использования `QueryLoad_Authors_SearchArgs`
  - **FeedProvider**: Исправлены вызовы `loadShoutsSearch` с правильной структурой `{ text, options }`
  - **Search Routes**: Обновлены роуты поиска для правильного использования новых API сигнатур
  - **Topics Route**: Исправлен `loadTopics` для корректной работы с `RoutePreloadFunc`
  - **Type Imports**: Добавлен импорт `QueryLoad_Authors_SearchArgs` в `public.ts`
  - **Все TypeScript ошибки исправлены**: Проект компилируется без ошибок типизации
  - **GraphQL API консистентность**: Все кешируемые загрузчики теперь используют единообразную структуру аргументов
...
- 🎨 **Улучшен layout ленты согласно Figma макету** - оптимизированы пропорции колонок
  - **Структура колонок**: Оптимизирована с `col-md-3 + col-md-6 + col-md-3` на `col-md-3 + col-md-7 + col-md-2`
  - **Центральная колонка**: Увеличена для лучшего отображения контента статей (7/12 вместо 6/12)
  - **Правая колонка**: Сжата для компактного отображения дополнительной информации (2/12 вместо 3/12)
  - **FeedSwitcher адаптация**: Возвращен к горизонтальному layout для лучшего использования пространства
    - Размеры: `padding: 1rem 1.5rem` и `font-size: 1.4rem` для комфортного использования
    - Ограничена ширина: `max-width: 400px` с `width: fit-content` для компактности
    - Счетчики: Увеличены до `2rem` высоты для лучшей читаемости
  - **FiltersContainer**: Возвращен горизонтальный layout с `justify-content: space-between`
    - FeedSwitcher слева, фильтры справа для оптимального использования пространства
    - Адаптивный переход на вертикальный layout только на мобильных устройствах
  - **Aside компоненты**: Адаптированы для узкой колонки
    - Заголовки: `1.8rem` на планшетах, `2rem` на больших экранах
    - Отступы: `1.5rem` на планшетах, `2rem` на больших экранах
  - **Responsive дизайн**: Улучшена адаптивность для всех размеров экрана
  - **Баланс контента**: Оптимальное соотношение между основным контентом и дополнительной информацией
