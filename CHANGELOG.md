...
### 🔄 Роутинг
- **🚀 SSR Fix**: Добавлен редирект для `/topic/[slug]` → `/topic/[slug]/shouts`
- **💻 Устойчивость**: Ссылки типа `/topic/social` теперь устойчивы к refresh
- Правильная SSR загрузка данных топика при прямом переходе
