...
- **✅ ИСПРАВЛЕНА отмена форматирования**: Кнопки теперь корректно отменяют форматирование при нажатии на активную кнопку
  - **Bug**: При нажатии на активную кнопку форматирования (жирный, курсив) форматирование не отменялось
  - **Root Cause**: 
    1. `hasFormatting` в `format/detection.ts` всегда возвращал `false` в конце функции
    2. Недостаточное логирование для диагностики проблем с определением активного состояния
    3. Логика `executeCommand` работала корректно, но не получала правильный статус от `hasFormatting`
  - **Fix**: 
    1. **Исправлена логика `hasFormatting`**: Убран неправильный `return false` в конце функции
    2. **Расширенное логирование**: Добавлены детальные логи в `executeCommand`, `removeInlineFormatting` и `hasFormatting`
    3. **Визуальные индикаторы**: Четкие эмодзи-маркеры (🗑️/➕) для операций удаления/применения форматирования
    4. **Пошаговая диагностика**: Логи для курсора, выделения, поиска родительских элементов
  - **Impact**: Полный цикл форматирования работает корректно - применение и отмена через кнопки тулбара
  - **Files**: `format/detection.ts`, `format/common.ts`, `format/inline.ts`

- **✅ ИСПРАВЛЕНО визуальное обновление кнопок тулбара**: Кнопки теперь показывают активное состояние в реальном времени
  - **Bug**: Кнопки форматирования не показывали активное состояние при перемещении курсора по форматированному тексту
  - **Root Cause**: 
    1. `trackSelectionAndCursor()` вызывался только при кликах, не при изменении выделения
    2. Недостаточная задержка в `handleAction` для стабилизации DOM после изменений
    3. Отсутствие глобального отслеживания изменений выделения
  - **Fix**: 
    1. **Глобальный `selectionchange` listener**: Добавлен в `onMount` для отслеживания всех изменений выделения
    2. **Увеличена задержка**: `setTimeout` в `handleAction` увеличен с 10ms до 50ms + дополнительный 5ms вызов
    3. **Двойное обновление**: Основное обновление + контрольное через 5ms для стабильности
    4. **Фильтрация событий**: `selectionchange` срабатывает только если выделение внутри редактора
  - **Impact**: Кнопки тулбара мгновенно отражают форматирование текста под курсором
  - **Files**: `SimpleRichEditor.tsx`, `format/detection.ts`
...
- **✅ ИСПРАВЛЕНО повторное форматирование**: Курсор больше не перескакивает при применении форматирования
  - **Bug**: При повторном применении форматирования (жирный, курсив) курсор перескакивал в начало предыдущего форматированного элемента
  - **Root Cause**: 
    1. Двойное восстановление фокуса в `handleAction` (setTimeout 50ms) + в функциях форматирования
    2. Конфликт между `restoreSelection()` и новым выделением пользователя
    3. Неправильное позиционирование курсора после inline форматирования
  - **Fix**: 
    1. **Убрано принудительное восстановление**: `handleAction` больше не восстанавливает фокус принудительно
    2. **Минимальная задержка**: Уменьшена задержка с 50ms до 10ms только для обновления состояния
    3. **Улучшено позиционирование**: Курсор после inline форматирования ставится ПОСЛЕ элемента, не внутри
    4. **Сохранение позиции**: Block форматирование сохраняет оригинальную позицию курсора в новом блоке
  - **Impact**: Решены все проблемы с последовательным форматированием, фокус сохраняется корректно
  - **Files**: `SimpleRichEditor.tsx`, `format/inline.ts`, `format/block.ts`

- **✅ ИСПРАВЛЕНА потеря фокуса**: Редактор сохраняет фокус после применения любого форматирования
  - **Bug**: После применения форматирования (Ctrl+B, Ctrl+I, заголовки) терялся фокус, нельзя было продолжать редактирование
  - **Root Cause**: Множественные попытки восстановления выделения конфликтовали друг с другом
  - **Fix**: Каждая функция форматирования сама восстанавливает выделение, `handleAction` не вмешивается
  - **Impact**: Плавное редактирование без потери фокуса
