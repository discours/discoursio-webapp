...
### 🖼️ Система изображений - Упрощение архитектуры

#### Убраны устаревшие квотер-оверлеи
- **Удалена поддержка shout параметра**: Параметр `?s=` больше не используется в системе кеширования
- **Упрощён API imageCache**: Убран параметр `shout` из `getCachedImageUrl()` и `getFileUrl()`
- **Исправлена функция getFileUrl**: Добавлена поддержка параметра `noSizeUrlPart` для оригинального размера
- **Обновлена документация**: Функциональность оверлеев помечена как устаревшая в `docs/image-caching.md`
- **Чёткое разделение ответственности**: Квотер используется только для изменения размера и WebP конвертации

#### Обратная совместимость getFileUrl
- **Поддержка width**: ✅ Полная поддержка изменения ширины через квотер
- **Параметр height**: ⚠️ Игнорируется (квотер поддерживает только width)
- **Параметр noSizeUrlPart**: ✅ Возвращает оригинальный размер без изменений
- **Улучшенная документация**: Добавлена таблица поддерживаемых параметров

#### Оптимизация Vercel OG API
- **Улучшена обработка обложек**: Обложки для OG изображений обрабатываются через квотер с размером 1200px
- **Правильные заголовки кеширования**: Добавлены `Content-Type` и `CDN-Cache-Control` для лучшего кеширования
- **Динамические URL**: Используется `process.env.VERCEL_URL` для корректных URL в разных окружениях

#### Документация
- **Обновлена image-caching.md**: Устаревшие функции помечены зачёркиванием и комментариями
- **Обновлена open-graph.md**: Убраны упоминания квотер-оверлеев, добавлены предупреждения об устаревших функциях
- **Чёткие примеры использования**: Показано правильное использование без устаревших параметров

### 🛠️ Технические изменения
- Убран параметр `shout` из всех TypeScript интерфейсов
- Закомментирован код обработки shout параметров для возможного восстановления
- Упрощена логика генерации URL изображений

### 🚀 UX Улучшения

#### Полностью убран флаш при загрузке фильтров ленты
- **Всегда видимые фильтры**: Выпадающие списки фильтров теперь отображаются всегда без любых условий
- **Убрана вся условная логика**: Удалена функция `shouldShowFilters()` и компонент `<Show>` который её использовал
- **Нулевая задержка**: Полностью устранён эффект "мигания" и "прыгающих" элементов интерфейса
- **Стабильная загрузка**: Все элементы фильтрации рендерятся сразу при загрузке компонента

### 🔧 Технические детали
- Полностью удалена функция `shouldShowFilters()` из `FeedFiltersControl.tsx`
- Убран компонент `<Show when={shouldShowFilters()}>` из рендера
- Упрощён API компонента: убраны неиспользуемые пропсы `type` и `mode`
- Удалён импорт `feedByMode` из `useFeed()` как неиспользуемый
- Фильтры теперь всегда присутствуют в DOM без условного рендеринга
