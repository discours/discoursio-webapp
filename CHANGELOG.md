...
#### Критическое исправление системы кеширования изображений
- 🖼️ **Исправлена проблема с треугольными заглушками**: Устранена несовместимость между логикой кеширования и обработкой ошибок
- 🔄 **Улучшена обработка ошибок в Image.tsx**: Добавлен корректный вызов callback `onError` родительского компонента
- ⚡ **Исправлено агрессивное кеширование браузера**: Добавлены timestamp параметры для принудительного обхода кеша
- 🛡️ **Упрощена логика повторных попыток**: Сокращено количество попыток с 3 до 2 для быстрого fallback на заглушки
- 🔧 **Обновлен middleware.js**: Расширена логика обработки параметров кеширования (v, t, retry)
- 📊 **Улучшено состояние загрузки**: Добавлено отслеживание состояния ошибок и успешной загрузки
- 🚫 **Исправлены ошибки загрузки**: Упрощена логика генерации URL с правильными параметрами обхода кеша
- 🖥️ **Исправлена SSR ошибка**: Добавлена проверка `isServer` в OfflineStatus и условная генерация timestamp
- 🔧 **Исправлена типизация**: Устранены ошибки TypeScript в обработчиках событий изображений
- 🎯 **Исправлен триггер перерисовки**: Добавлен сигнал `loaded` в Image.tsx для принудительного обновления UI при успешной загрузке
- 🔄 **Исправлена логика сброса ошибок**: ArticleCard теперь корректно сбрасывает состояние ошибки изображения при успешной загрузке

### 👨‍💻 Технические подробности
- **Исправлен флоу обработки ошибок**: 
  1. `ArticleCard.tsx` → `Image.tsx` (передает onError callback)
  2. `Image.tsx` → `getCachedImageUrl()` (генерирует URL с фиксированной версией)
  3. При ошибке → `handleImageError()` (корректно вызывает props.onError)
  4. `ArticleCard.tsx` → показывает CoverImage заглушку (состояние обновляется)
- **Оптимизировано кеширование**: Использование константы `CACHE_VERSION = '1.0.0'` вместо `Date.now()`
- **Упрощена генерация URL**: Убрана сложная логика CDN трансформации, используются оригинальные URL с параметрами
- **Улучшена типизация**: Исправлены ошибки TypeScript для обработчиков событий в SolidJS
- **Упрощен middleware**: Убраны избыточные проверки, оставлена только обработка изображений
- **SSR совместимость**: Добавлены проверки `isServer` и `typeof window !== 'undefined'`
- **Реактивность изображений**: Добавлен сигнал `loaded` в Image.tsx для отслеживания состояния загрузки
- **Визуальная индикация**: Изображения показывают opacity 0.5 во время загрузки и 1.0 после успешной загрузки
- **Сброс состояния ошибок**: ArticleCard.onLoad теперь сбрасывает `isCoverImageLoadError(false)` при успешной загрузке

### 👨‍💻 Технические подробности
- Реализована комплексная стратегия кеширования на трех уровнях:
  1. **Браузер**: Скрипт `image-refresh.js` с автоматическим обновлением изображений
  2. **CDN Vercel**: Настройки в `vercel.json` и `middleware.js` для управления кешированием
  3. **Service Worker**: Оптимизированные стратегии кеширования для разных типов ресурсов
- Добавлен параметр `v={timestamp}-{random}` к URL изображений для гарантированного предотвращения кеширования
- Реализована функция `handleImageError` с несколькими попытками перезагрузки изображений при ошибке
- Оптимизирована стратегия кеширования в Service Worker с дифференцированным подходом к разным типам ресурсов
- Добавлены настройки кеширования в vercel.json с условиями для разных типов запросов
- Создан скрипт `image-refresh.js` с расширенным API для управления кешем изображений
- Добавлен middleware для интеллектуальной установки заголовков кеширования в зависимости от типа запроса
- Создан API-эндпоинт `/api/purge-cache` для ручной и автоматической очистки кеша
- Настроена cron-задача для периодической очистки кеша каждые 6 часов
