...
- **✅ ИСПРАВЛЕНО отображение Plus-menu в редакторе**: Plus-menu теперь корректно показывается на пустых строках
  - **Problem**: Plus-menu (кнопка "+") не появлялся в редакторе при установке курсора на пустую строку, что критически ухудшало UX
  - **Root Cause**: 
    1. **CSS проблема**: Класс `.visible` устанавливал `opacity: 0`, делая меню полностью невидимым даже когда логика показа работала корректно
    2. **Отсутствие отладки**: Не было логирования для диагностики условий показа меню
  - **Fix**: 
    1. **Исправлен CSS**: `opacity: 0` → `opacity: 0.6` в классе `.visible` для базовой видимости
    2. **Добавлено отладочное логирование**: В `shouldShowPlusMenu()`, `getPlusMenuPosition()`, `isCursorOnEmptyLine()` для диагностики
    3. **Сохранена анимация**: Класс `.appearing` по-прежнему устанавливает `opacity: 1` для полной видимости
  - **Impact**: 
    - ✅ Plus-menu теперь видим на пустых строках
    - ✅ Улучшен UX добавления контента (ссылки, изображения, видео)
    - ✅ Добавлена диагностика для будущих проблем
  - **Files**: `src/components/SimpleRichEditor/menu/PlusMenu.module.scss`, `src/components/SimpleRichEditor/handlers/ui.ts`
