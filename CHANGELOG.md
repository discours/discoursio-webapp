...
- **⚡ Улучшенная логика Loading**: Добавлен флаг `isLoading` в `HomeViewProps` для корректного управления состоянием загрузки
- **🛡️ Защита от пустых состояний**: Новая логика `shouldShowLoading()` предотвращает показ Loading при наличии любого контента
- **🔄 Стабильная гидратация**: Исправлена проблема с SolidJS гидратацией и переинициализацией ресурсов
- **📊 Комплексная проверка данных**: Loading показывается только при отсутствии всех типов контента (статьи, рейтинги, темы)
