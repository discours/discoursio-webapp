...
### 🐛 FIX: Squib (Подвёрстка) Menu Visibility & Positioning
- **✅ Исправлено меню редактора подвёрстки**: Теперь появляется автоматически над блоком
  - **КРИТИЧНО**: Добавлены `data-align`, `data-bg`, `data-squib-id`, `data-type` в `ALLOWED_ATTR` (sanitize.ts)
  - Атрибуты подвёрстки теперь не удаляются при санитизации HTML
  - Меню автоматически появляется после создания подвёрстки (команда `squib`)
  - Динамическое позиционирование меню над блоком подвёрстки
  - Функция `calculateSquibMenuPosition` для вычисления координат
  - Обработчик кликов ищет элементы по `[data-align]`
  - Подвёрстки определяются по наличию атрибута `data-align`
  - Обновлены `media/handlers.ts` и `lib/squib-helpers.ts`
  - Добавлены визуальные стили для кнопки закрытия (×)
  - Разделены стили для табов (`.tabButton`) и кнопок форматирования (`.button`)
  - Улучшена структура header с правильным группированием элементов
  - Добавлен hover effect и title для кнопки закрытия
