...
- **🔧 Исправлена критическая ошибка DOM**: Устранена ошибка "NotFoundError: insertBefore" в SimpleRichEditor при навигации
- **🔧 Исправлена загрузка свежих публикаций**: Улучшена логика обновления данных в ленте с кешированием и тайм-аутами
- **🔧 Исправлена ошибка гидрации**: Добавлен ClientOnly wrapper для FeedView для предотвращения конфликтов SSR/CSR
- **⚡ Оптимизирована инициализация ленты**: Улучшена синхронизация между SSR и клиентскими данными
- **🔧 Добавлены защитные проверки**: Предотвращение race conditions при смене режимов ленты
- **🔧 Исправлен режим "Свежие"**: Улучшена логика URL routing и автоматической загрузки для режима recent
- **⚡ Добавлена принудительная загрузка**: Клиент автоматически загружает данные если SSR fallback недоступен
- **🎨 Изолированная загрузка постов**: При переключении режимов только список статей показывает loading, sidebar и switcher остаются на месте

#### Мерцающие скелетоны вместо Loading
- **✨ Новые мерцающие скелетоны**: Создан компонент `ArticleCardSkeleton` с shimmer анимацией для красивой загрузки
- **🎨 Умная замена Loading**: Вместо Loading поверх всего контента показываются скелетоны на месте будущих карточек
- **🚫 Убран Loading с главной**: Полностью удален Loading fallback из HomeView и main.tsx для лучшего UX
- **📱 Адаптивные скелетоны**: Разные размеры скелетонов (small/medium/large) для разных типов карточек
- **🌙 Поддержка темной темы**: Скелетоны автоматически адаптируются к svetlой и темной теме

#### Исправления гидратации и состояния загрузки
- **🔧 Улучшенная валидация данных**: Row5 теперь проверяет не только длину массива, но и валидность каждой статьи (article?.id)
- **⚡ Исправлена инициализация ресурсов**: Используется `undefined` как initial value вместо пустых массивов для правильной SolidJS гидратации
- **🚀 Предотвращение мисматчей гидратации**: Ресурсы корректно инициализируются только с валидными SSR данными
- **🛡️ Защита от пустых состояний**: Дополнительные проверки безопасности в HomeView и Row5

#### Исправления DOM ошибок при навигации
- **🔧 Исправлена ошибка "insertBefore"**: Устранена ошибка DOM манипуляций при переходах между страницами
- **⚡ Исправлены асинхронные эффекты**: Убран неправильный `createEffect(async () => {...})` из feed роута
- **🛡️ Проверка состояния ресурсов**: Добавлены проверки `!resource.loading && !resource.error` перед обновлением сигналов
- **🎯 Исправлена инициализация сигналов**: SSR данные теперь корректно инициализируют сигналы контекста в createEffect
- **🚀 Убрана блокирующая загрузка**: Удален Show wrapper который блокировал отображение контента при наличии данных

#### Исправления отображения ленты (/feed)
- **🔧 Упрощена передача данных**: Убраны промежуточные сигналы в FeedPage, данные передаются напрямую из SSR
- **⚡ Улучшена логика инициализации**: FeedView теперь правильно инициализирует контекст feed с SSR данными
- **🎯 Исправлена логика sortedFeed**: Приоритет данных из контекста, fallback на props при пустом контексте
- **🛡️ Добавлены проверки isEmpty**: Учитывается состояние `currentFeed.isEmpty` при выборе источника данных
- **Правильное использование createMemo в SolidJS**: Убрал избыточные createMemo для простых операций
  - `shouldShowPlaceholder`, `placeholderType`, `isLoading` в FeedView - простые функции
  - Оставил createMemo только для сложных вычислений: `sortedFeed`, `feedByMode` (сложная логика выбора feed)
  - ⚠️ **Исправлен mode**: Теперь реактивный сигнал + createEffect для отслеживания URL, а не createMemo
  - Согласно документации SolidJS: createMemo только для кеширования сложных вычислений

#### Синхронизация переводов метаданных
- **🔧 Исправление использования t() в метаданных**: Исправлено прямое использование строки 'Discours' вместо функции t('Discours') в PageLayout.tsx
- **🌐 Проверка fallback системы**: Подтверждено, что система переводов правильно работает с fallback для одинаковых ключ-значение пар
- **📊 Валидация переводов**: Проверена синхронизация всех используемых ключей переводов между русской и английской версиями
- **⚡ Оптимизация переводов**: Убеждены что технические значения (URL, цвета, размеры) правильно не переводятся

#### Оптимизация SolidJS реактивности
- **⚡ Устранение избыточных createMemo**: Заменена функция `content()` с createMemo на обычную функцию для простых условий
- **🎯 Правильное использование createEffect**: Применен паттерн `createEffect(on([...], ...))` для отслеживания изменений метаданных
- **📚 Следование SolidJS best practices**: Оставлены createMemo только для сложных вычислений (generateOGMetadata, getPageKeywords)
- **🚀 Улучшенная производительность**: Устранены ненужные мемоизации простых геттеров согласно документации SolidJS

#### Устранение деградации к состоянию загрузки
- **🚨 Критическое исправление**: Устранена проблема "деградации" главной страницы обратно к состоянию Loading после первоначальной загрузки  
- **⚡ Улучшенная логика**: Замена `shouldShowLoading()` на `shouldShowSkeletons()` для более точного контроля
- **🛡️ Защита от пустых состояний**: Скелетоны показываются только при полном отсутствии данных
- **🔄 Стабильная гидратация**: Исправлена проблема с SolidJS гидратацией и переинициализацией ресурсов
- **🎯 Упрощенная структура Row5**: Убран отдельный компонент Row5Skeleton, теперь ArticleCardSkeleton используется прямо в Row5 с правильной структурой (3+1+2)

#### Техническая архитектура
- **🎯 Оптимизированные условия**: Условие `when={!shouldShowLoading()}` заменило простую проверку наличия статей
- **🔗 Интеграция с ресурсами**: Передача состояния загрузки `featuredShouts.loading || topData.loading` из родительского компонента
- **⚡ Предотвращение мерцаний**: Контент остается видимым даже при фоновой загрузке дополнительных данных
- **🛠️ Улучшенная отладка**: Защищенные проверки nullable значений для предотвращения ошибок линтера
- **🔧 Исправлена ошибка refresh**: Устранены ошибки `Cannot read properties of undefined (reading 'layout')` и `(reading 'seo')` в ArticleCard
- **🛡️ Комплексная защита от undefined**: Добавлены безопасные проверки для всех обращений к props.article в компонентах
- **🎨 Улучшенное отображение**: TopicsNav и Hero теперь показываются независимо от состояния загрузки данных

### 🔧 Исправления SSR (Server-Side Rendering)
- **Приоритет SSR данных в FeedView**: Изменён порядок приоритета - сначала SSR данные (props), затем контекст
- **Упрощена логика HomePage**: Убрана сложная логика с createResource, данные используются напрямую из props
- **Добавлено логирование SSR**: Для отладки загрузки и передачи данных от сервера к клиенту
- **Оптимизирована инициализация**: Контекст загружает данные только если нет SSR данных
- **Оптимизирована SSR загрузка**: Исправлен timeout при загрузке главной страницы и feed
  - Увеличен SSR timeout с 8 до 15 секунд для загрузки всех режимов feed  
  - **Главная страница**: SSR загружает только featured shouts, остальные данные загружаются асинхронно
  - **Feed лента**: SSR загружает ВСЕ режимы (recent, hot, top) для мгновенного переключения без ожидания
  - Автоматическая инициализация контекста feed с SSR данными для всех режимов
  - recentComments и unratedShouts загружаются асинхронно на клиенте
  - Приоритетная система данных: SSR → context → client loaded → fallback
- **Восстановлена логика смена режима**: Исправлен сброс пагинации (offset: 0) при смене режима ленты
  - Используется batch() для группировки операций сброса состояния
  - Сбрасываются и myRates при смене режима
- **Исправлены стили ссылок в Aside секции**: Приведены к общему стилю проекта
  - Убран черный цвет и синее подчеркивание ссылок в aside секциях
  - Используются глобальные CSS переменные `--link-color` и `--link-hover-color`
  - Заменен устаревший `loadShoutsMyRates` на современный хук `useShoutsMyRates`
- **🐛 Исправлены критичные проблемы с лентой**: 
  - Исправлено определение режима ленты из URL (`/feed` -> `recent`)
  - Добавлен SSR fallback для мгновенного отображения данных
  - Исправлена ошибка гидрации через клиентскую инициализацию контекста
  - Убраны лишние borders в стилях ссылок
- **🔧 Исправлена передача SSR данных в компоненты**:
  - Решена проблема с пустыми данными в FeedView несмотря на успешную загрузку в SSR
  - Добавлена безопасная обработка промисов в props.data для предотвращения гонки условий
  - Исправлена ошибка гидрации в FeedFiltersControl через проверку клиентского окружения
  - Улучшены логи инициализации для отладки передачи данных
...
#### Критическое исправление системы отображения изображений
- 🖼️ **Исправлена проблема с треугольными заглушками**: Устранена несовместимость между логикой кеширования и обработкой ошибок
- 🔄 **Исправлена обработка ошибок в режиме ленты**: Добавлены отсутствующие обработчики `onError` и `onLoad` для изображений в `isFeedMode`
- ⚡ **Добавлен триггер перерисовки**: Введен сигнал `loaded` в Image.tsx для принудительного обновления UI после загрузки
- 🛡️ **Исправлено сбрасывание состояния ошибок**: ArticleCard теперь корректно сбрасывает `isCoverImageLoadError` при успешной загрузке
- 🔧 **Улучшен кавер в статьях**: Увеличен размер изображения с 800px до 1200px и добавлены обработчики ошибок
- 📊 **Унифицирована обработка изображений**: Все компоненты теперь используют одинаковую логику fallback на заглушки
- 🚫 **Исправлены ошибки загрузки**: Упрощена логика генерации URL с правильными параметрами обхода кеша
- 🎨 **Добавлена визуальная индикация загрузки**: Изображения показываются с opacity 0.5 во время загрузки
