...
test.describe('Аутентификация и доступ к защищенным страницам', () => {
  test('@smoke Должна перенаправлять на авторизацию при попытке доступа к защищенным страницам', async ({
    ...
    const hasAuthModal = await page.$('[data-testid="auth-modal"], .auth-modal, .modal[aria-label*="auth"], .modal[aria-label*="Auth"]') !== null
    const hasLoginForm = await page.$('input[type="email"], input[name="email"], form[action*="login"], .login-form') !== null
  ...
  test('@smoke Должна отображаться форма входа при клике на кнопку "Войти"', async ({ page }) => {
    ...
    try {
      ...
      const hasAuthForm = await page.waitForSelector([
