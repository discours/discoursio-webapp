...
export default (props: RouteSectionProps<{ shouts: Shout[]; topics: Topic[] }>) => {
  ...
  // /feed/:mode: all | featured | unfeatured | followed | discussed | coauthored | unrated
  const mode = createMemo(() => params.mode || 'all')

  // /feed/:mode/:order: recent | hot | top
  const order = createMemo(() =>
    params.order === 'hot' ? 'last_reacted_at' : params.order === 'top' ? 'rating' : undefined
  )
